...
function updateServerList(){
  ...
  var data_updated_time = new Date();
  console.log("Data updated at " + data_updated_time.getHours() + ":" + data_updated_time.getMinutes() + ":" + data_updated_time.getSeconds());
  console.table(sdata["online_servers_list"]);
...
function updatePlayersInLobby(){
  ...
  sdata["online_players_list"].forEach(function(p){
    if(playersInServers.includes(p) || bots.includes(p)){
      //Ignore
    }else{
      lobbyPlayers.push(p);
    }
  ...
  item = '<div class="playerList hidden">';
  if(sdata["online_players_list"].length > 0){
    sdata["online_players_list"].forEach(function(p){
      if(playersInServers.includes(p) || bots.includes(p)){
        //Ignore
      }else{
    ...
    item += '<div class="playerItem">'
         +    '<div class="playerItemInner"><p><i>Lobby is empty.</i></p></div>'
         +  '</div>';
  }
  item += '</div>';
  $("#COMMUNITY-SRVLST").append(item);
...
function updateCurrentlyOnline(){
  var n = 0;
  var i = 0;
  while(i < sdata["online_players_list"].length){
    if(bots.includes(sdata["online_players_list"][i])){
      //Ignore
    }else{
      n++;
    }
